...
for(i in rcplist){
  ...
  # # to test the loop - otherwise uncomment it with #
  # i = rcplist[2]
  ...
  df.refbau <- rbind(df.bauwt, df.bau, df.sa) %>% 
    ...
    #mutate(climate = i) # data has already info about climate --> column scenario
...
# df.filteredforplot %>%
#   ggplot(aes(x=scenario, y= scenario, fill=factor(regime)))+ # you cannot plot scenario (x) against scenario (y), x and y need to be different
#   geom_bar(stat="identity")+
#   labs(fill="Year")+
#   facet_wrap(~ policy)
